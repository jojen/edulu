...
<%--@elvariable id="actor" type="org.jojen.wikistudy.domain.Actor"--%>
<%--@elvariable id="role" type="org.jojen.wikistudy.domain.Role"--%>
<c:choose>
        ...
        <c:if test="${not empty actor.roles}">
          <ul>
            <c:forEach items="${roles}" var="role">
              <li>
                <a href="/movies/${role.course.id}"><c:out value="${role.course.title}" /> as <c:out value="${role.name}" /> in <c:out value="${role.course.year}"/></a><br/>
              </li>
            </c:forEach>
          </ul>
        </c:if>
