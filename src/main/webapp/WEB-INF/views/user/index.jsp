...
<%--@elvariable id="user" type="org.jojen.wikistudy.domain.User"--%>
<%--@elvariable id="recommendations" type="java.util.List<org.jojen.wikistudy.domain.dep.MovieRecommendation>"--%>
<%--@elvariable id="recommendation" type="org.jojen.wikistudy.domain.dep.MovieRecommendation"--%>
<html>
<head>
    <title>Profile</title>
</head>
        ...
        <c:when test="${not empty user.friends}">
            ...
            <ul class="friends-list">
                <c:forEach items="${user.friends}" var="friend">
                    <li>
                        <a class="friend-image" href="<c:url value="/user/${friend.login}" />"><img
                                src="<c:url value="/images/profile-placeholder-small.png" />"/></a>

                        <div class="friend-info">
                            <h3><a href="<c:url value="/user/${friend.login}" />"><c:out value="${friend.name}"/></a>
                            </h3>
            ...
            <c:choose>
                <c:when test="${not empty ratings}">
                    <c:forEach items="${ratings}" var="rating">
                        <c:set var="course" value="${rating.course}"/>
                        <c:set var="stars" value="${rating.stars}"/>
                        <li>
                            <h4><a href="<c:url value="/movies/${course.id}" />"><c:out value="${course.title}"/>
                                (${course.year}) - &quot;${rating.comment}&quot;</a>
            ...
            <c:choose>
                <c:when test="${not empty recommendations}">
                    <c:forEach items="${recommendations}" var="recommendation">
                        <c:set var="course" value="${recommendation.course}"/>
                        <c:set var="stars" value="${recommendation.rating}"/>
                        <li>
                            <h4><a href="<c:url value="/movies/${course.id}" />"><c:out value="${course.title}"/>
                                (${course.year}) - &quot;${course.tagline}&quot;</a>
                                <img class="rating" src="/images/rated_${stars}.png" alt="${stars} stars"/>
                            </h4>
                        </li>
                    </c:forEach>
